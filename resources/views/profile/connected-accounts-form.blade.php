<x-action-section>
    ...
    <x-slot name="description">
        {{ __('Connect your social media accounts to ease up your login process.') }}
    </x-slot>

    <x-slot name="content">
        <div class="p-4 text-sm font-medium text-red-500 border-l-4 border-red-600 rounded bg-red-500/10">
                        ...
                        @if (! is_null($account))
                            <div class="flex items-center space-x-6">
                                @if (Laravel\Jetstream\Jetstream::managesProfilePhotos() && ! is_null($account->avatar_path))
                                    <button class="text-sm text-gray-500 cursor-pointer ms-6 hover:text-gray-700 focus:outline-none" wire:click="setAvatarAsProfilePhoto({{ $account->id }})">
